...
search distribution D(ψ) over policy parameterizations for policy
π(θ, s) . This implementation also
takes an initial search distribution
parameterization ψ , the log search
likelihood gradient ∇logp(ψ, θ) ,
a policy evaluation function U ,
and an iteration count k_max . In
each iteration, m parameterization samples are drawn and are
used to estimate the search gradient. This gradient is then applied
with a step factor α . We can use
Distributions.jl to define D(ψ) .
...
an isotropic multivariate Gaussian
search distribution with mean ψ
and covariance σ 2 I over policy parameterizations for policy π(θ, s) .
This implementation also takes a
policy evaluation function U , a step
factor α , and an iteration count
k_max . In each iteration, m/2 parameterization samples are drawn and
mirrored, and are then used to estimate the search gradient.
    """
    # initial mean
    ψ::Any
    σ::Any
