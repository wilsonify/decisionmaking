...
@show(typeof(G)) # == LightGraphs.SimpleGraphs.SimpleDiGraph{Int64})
@show(nv(G)) # == 3)# number of nodes (also called vertices)
@show(outneighbors(G, 1)) # == [2]) # list of outgoing neighbors for node 1
@show(inneighbors(G, 1)) # == []) # list of incoming neighbors for node 1
...
μ, σ = 5.0, 2.5
dist = Distributions.Normal(μ, σ) # create a normal distribution
@show(dist)
...
@show(dist) # == FullNormal(dim:2, μ:[1.0, 2.0], Σ:[1.0 0.5; 0.5 2.0]))
