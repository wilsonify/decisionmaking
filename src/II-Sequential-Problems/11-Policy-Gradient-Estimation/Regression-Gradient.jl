struct RegressionGradient
    ...
    A method for es        timating a policy gradient using
        ...
        π(θ, s) , and policy parameteriza        tion vector θ . Policy variation vec        tors are generated by normalizing
        normally-distributed samples and
        scaling by a perturbation scalar δ .
        A total of m parameter perturba        tions are generated, and each is
        evaluated in a rollout from an ini        tial state drawn from b to depth d
        and compared to the original pol        icy parameterization.
    """
    𝒫::Any # problem
